...
function convertNamesToChartData(names) {
    const chartData = {};

    for (const key in names) {
        chartData[key] = Object.entries(names[key]).map(([employeeName, value]) => ({ label: employeeName, value }));
    }
...
function getEmployeeChartContent(employees) {
    ...
    employees.forEach(employee => {
        const { name, gender, workload } = employee
        ...
        if (gender === 'female' && workload <= 30) {
            if (!names.femalePartTime[name]) {
                names.femalePartTime[name] = 1
            } else {
                names.femalePartTime[name] += 1
            }
        }

        if (gender === 'male' && workload === 40) {
            if (!names.maleFullTime[name]) {
                names.maleFullTime[name] = 1
            } else {
                names.maleFullTime[name] += 1
            }
        }
    ...
    const chartData = convertNamesToChartData(names);
...
console.log(JSON.stringify(dtoOut));
